...
fn create_dir() -> std::io::Result<()>{
    ...
    let create_gen_files = cur_dir.into_os_string().into_string().unwrap().to_string() + "/gen_files";
    ...
    create_1(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
    create_empty(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
    
    create_1000_diff(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
    create_1000_1(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();

    generate_100000(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
...
fn read() -> std::io::Result<()>{
    ...
    let cur_dir = std::env::current_dir()?.into_os_string().into_string().unwrap().to_string() + "/gen_files";
    ...
    io::stdin().read_line(&mut input).expect("unable to parse input");
        ...
        if input.eq("1\n") {
            ...
            hash(temp_dir.clone() + "/empty_file.txt", 0).map_err(|err| println!("{:?}",err)).ok();
            break;
        }
        else if input.eq("2\n"){
            if !file_exists(cur_dir.clone()+ "/single_char2.txt") || !file_exists(cur_dir.clone() + "/single_char2.txt"){
                println!("File doesn't exists, Creating file!");
                create_1(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
                println!("File created!");
            }
            ...
            print!("First file: ");
            hash(temp_dir.clone() + "/single_char1.txt", 0).map_err(|err| println!("{:?}",err)).ok();

            print!("Second file: ");
            hash(temp_dir.clone() + "/single_char2.txt", 0).map_err(|err| println!("{:?}",err)).ok();

            break;
        }
        else if input.eq("3\n"){
            if !file_exists(cur_dir.clone() + "/1000_rand1.txt") || !file_exists(cur_dir.clone() + "/1000_rand2.txt") {
                println!("File doesn't exists, Creating file!");
                create_1000_1(cur_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
                println!("File created!");
            }

            print!("First file: ");
            hash(temp_dir.clone() + "/1000_rand1.txt", 0).map_err(|err| println!("{:?}",err)).ok();
            
            print!("Second file: ");
            hash(temp_dir.clone() + "/1000_rand2.txt", 0).map_err(|err| println!("{:?}",err)).ok();

            break;
        }
        else if input.eq("4\n"){
            if !file_exists(cur_dir.clone() + "/1000_rand_diff_1.txt") || !file_exists(cur_dir.clone() + "/1000_rand_diff_2.txt"){
                println!("File doesn't exists, Creating file!");
                create_1000_diff(temp_dir.clone()).map_err(|err| println!("{:?}",err)).ok();
                println!("File created!");
            }
            print!("First file: ");
            hash(temp_dir.clone() + "/1000_rand_diff_1.txt", 0).map_err(|err| println!("{:?}",err)).ok();
            
            print!("Second file: ");
            hash(temp_dir.clone() + "/1000_rand_diff_2.txt", 0).map_err(|err| println!("{:?}",err)).ok();
        ...
        hash(temp_dir.clone() + "/100000_rand_pairs.txt", 1).map_err(|err| println!("{:?}",err)).ok();
        ...
        break;  
            ...
            io::stdin().read_line(&mut location).expect("Unable to parse input!");
            location.pop();
            ...
            io::stdin().read_line(&mut hand).expect("Unable to parse input!");
            hand.pop();
            hand_input(hand)?;
            break;
        }
